...
class DataMapper<I : Any, O : Any>(private val inType: KClass<I>, private val outType: KClass<O>) :
    Mapper<I, O> {

    companion object {
        inline operator fun <reified I : Any, reified O : Any> invoke() =
            DataMapper(I::class, O::class)

        fun <I : Any, O : Any> arrayToListMapper(mapper: Mapper<I, O>) =
            object : Mapper<Array<I>, List<O>> {
    ...
    private fun argFor(parameter: KParameter, data: I): Any? {
        ...
        return try {
            fieldMappers[parameter.name]?.invoke(value) ?: value
        } catch (e: Exception) {
            println("${e.message} of ${parameter.name} on ${data.javaClass.simpleName}")
            e.printStackTrace()
    ...
    fun <T : Any> targetParameterSupplier(
        parameterName: String,
        mapper: targetParameterSupplier<T>
    ): DataMapper<I, O> = apply {
    ...
    override fun invoke(data: I): O = with(outConstructor) {
        callBy(parameters.associateWith {
            println("Map >> ${it.name} of  ${data.javaClass.simpleName}")
            argFor(it, data)
        })
    }
}
